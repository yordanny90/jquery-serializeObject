...
return c}});$.extend($,{serializeObject_util:serializeObject_util,formData_getObject:q,formData_getArray:function(a){if(!(a instanceof FormData))return null;var b=[];$.each(Array.from(a.entries()),function(c,d){d[1]instanceof Object||b.push({name:d[0],value:d[1]})});return b},formData_getFiles:r,formData_intoJSONFile:function(a,b){if(!(a instanceof FormData))return null;if(null==b||""===(b=String(b)))b="data.json";var c=k(r(a));a=JSON.stringify(q(a));a=new File([a],"data.json",{type:"text/json"});
c.append(b,a);return c},formData_intoJSON:function(a,b){if(!(a instanceof FormData))return null;if(null==b||""===(b=String(b)))b="data.json";var c=k(r(a));a=JSON.stringify(q(a));c.append(b,a);return c},formData_fromArray:k,formData_merge:function(a,b){for(var c=[],d=1;d<arguments.length;++d)c[d-1]=arguments[d];a instanceof FormData||(a=new FormData);$.each(c,function(h,g){g instanceof FormData&&$.each(Array.from(g.entries()),function(u,e){a.append(e[0],e[1])})});return a}});var m=function(a,b,c){c||
(c=$());a=$(a);if(a.is("fieldset")){var d=a.parentsUntil("form","fieldset").last();d.length&&(a=d)}if(!c.filter(a).length){c=c.add(a);try{var h={},g=l(a).filter("[form2json]:input").not('[form2json=""]');g.each(function(e,f){e=$(f).attr("form2json");h[e]||(h[e]={form:document.getElementById(e),inputs:[]});h[e].inputs.push(f)});g.val("");$.each(h,function(e,f){e=$(f.form).filter("form,fieldset");f=$(f.inputs);if(e.length&&f.length)try{c=m(e,b,c)||c,f=f.filter(function(z,D){return""==D.value}),f.length&&
f.val(JSON.stringify(e.serializeMakeObject(!1)))}catch(z){console.error(z)}});var u=g.filter(function(e,f){return""==f.value});u.length&&console.debug("missing",u)}catch(e){console.error(e)}return c}};$(document).on("submit","form",function(a){$(this).form2json()});$(document).on("form2json","form",function(a){m(this,a)});$(document).on("form2json-fieldset","fieldset",function(a){a.stopPropagation();m(this,a)});$(document).on("form2json-input",":input[form2json]",function(a){var b=$(this);var c=b.attr("form2json");
null==c||""===c?b=void 0:(b.val(""),b=$(document.getElementById(c)),b=b.is("form,fieldset")?b:void 0);b&&m(b,a)})})();
